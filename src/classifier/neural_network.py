...
class NeuralNetwork:
    ...
    def init_model(self, config, no_features):
        ...
        self.w_0 = np.zeros((self.no_neurons, no_features)).tolist()
        for (j, i), w_j_i in zip(product(range(self.no_neurons), range(no_features)), (np.random.rand(no_features*self.no_neurons)-0.5)):
            self.w_0[j][i] = self.add_shift(w_j_i, config['precision'])

        self.w_1 = np.zeros((1, self.no_neurons)).tolist()
        for w_0_i, i in zip(np.random.rand(self.no_neurons)-0.5, range(self.no_neurons)):
    ...
    def format_weights(self, as_zokrates, from_variable):
        ...
        if from_variable:
            ...
            for j in range(self.no_neurons):
                weights_str += f'    u64[{len(self.w_0[j])}] w_0_{j} = {self.w_0[j]}\n'
            weights_str += f'    u64[{self.no_neurons}] w_1_0 = {self.w_1[0]}\n'
            weights_str += f'    u64[{self.no_neurons}] b_0 = {[0]*self.no_neurons}\n'
            weights_str += f'    u64 b_1 = 0'
    ...
    def set_weights(self, weights):
        weights = weight_iterator(weights)

        self.w_0 = np.zeros((self.no_neurons, self.no_features)).tolist()
        for (j, i), w_j_i in zip(product(range(self.no_neurons), range(self.no_features)), weights):
            self.w_0[j][i] = w_j_i

        self.b_0 = [0]*self.no_neurons
        for i, b_0_i in zip(range(self.no_neurons), weights):
            self.b_0[i] = b_0_i

        self.w_1 = np.zeros((1, self.no_neurons)).tolist()
        for i, w_0_i in zip(range(self.no_neurons), weights):
            self.w_1[0][i] = w_0_i

        self.b_1 = list(weights)
